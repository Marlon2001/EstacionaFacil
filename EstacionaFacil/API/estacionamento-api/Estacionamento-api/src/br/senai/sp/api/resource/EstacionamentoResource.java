...
public class EstacionamentoResource {
	...
	public ResponseEntity<Movimentacao> gerarOrcamento(@PathVariable Long cod_movimentacao,
		...
		try {
			...
			if (movimentacao.getTipo().equals("A")) {
				precos = precosRepository.findByDataSaidaIsNull();
			...
			URI uri = ServletUriComponentsBuilder.fromCurrentRequestUri().path("/{cod_movimentacao}")
					.buildAndExpand(movimentacao.getCodMovimentacao()).toUri();
	...
	public ResponseEntity<Movimentacao> salvar(@RequestBody Movimentacao movimentacao, HttpServletResponse response) {
		...
		int veiculo;
		veiculo = veiculoRepository.findyByPlaca(movimentacao.getPlaca());
		if (veiculo == 0) {
	...
	public ResponseEntity<Movimentacao> gerarSaida(@PathVariable Long cod_movimentacao, HttpServletResponse response) {
		...
		try {
			...
			Double totalAPagar = 0.00;
			Precos precos;
			if (movimentacaoSalva.getTipo().equals("A")) {
				precos = precosRepository.findByDataSaidaIsNull();
				cobranca = new Cobrancas();
				totalAPagar = cobranca.getValorPagar(tempoPermanencia, precos);
			}
